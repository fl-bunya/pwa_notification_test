...
const registerServiceWorker = async () => {
  if ('serviceWorker' in navigator) {
    try {
      ...
      // サーバから公開キーを取得
      const vapidPublicKey = await (await fetch('https://pwa-notification-test.onrender.com/api/vapidPublicKey')).text();
...
const checkServiceWorkerRegistered = async () => {
  ...
  if (swReg) {
    ...
    if (subscription) {
      ...
      cmdArea.textContent = `curl https://pwa-notification-test.onrender.com/api/curlPushTest -X POST -H "Content-Type: application/json" -d '${jsonSub}'`;
